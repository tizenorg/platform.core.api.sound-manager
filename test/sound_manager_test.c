...
enum {
	...
	CURRENT_STATUS_GET_MASTER_MAX_VOLUME,
...
void _interpret_main_menu(char *cmd)
{
	...
	if (strncmp(cmd, "mgx", 3) == 0)
		g_menu_state = CURRENT_STATUS_GET_MASTER_MAX_VOLUME;
...
void display_sub_basic()
{
	...
	g_print("mgx. *Get Master Max Volume \t");
...
static void displaymenu()
{
	...
	else if (g_menu_state == CURRENT_STATUS_GET_MASTER_MAX_VOLUME)
		g_print("*** press enter to get master max volume level\n");
...
static void interpret(char *cmd)
{
	...
	case CURRENT_STATUS_GET_MASTER_MAX_VOLUME: {
		int max_level;
		if (sound_manager_get_master_max_volume(&max_level) != SOUND_MANAGER_ERROR_NONE)
			g_print("failed to get master max volume\n");
		else
			g_print("the max volume of master is %d\n", max_level);
