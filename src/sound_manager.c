...
int sound_manager_set_session_interrupted_cb (sound_session_interrupted_cb callback, void *user_data)
{
	...
	if (g_session_interrupt_cb_table.is_registered == 0) {
		mm_sound_focus_set_session_interrupt_callback((mm_sound_focus_session_interrupt_cb)_focus_session_interrupt_cb, NULL);
		...
		if (ret) {
			if (MM_ERROR_NONE != mm_sound_focus_unset_session_interrupt_callback())
				LOGW("failed to unset session interrupt callback");
			goto finish;
		}
		g_session_interrupt_cb_table.is_registered = 1;
...
int sound_manager_unset_session_interrupted_cb (void)
{
	...
	if (g_session_interrupt_cb_table.user_cb) {
		if(MM_ERROR_NONE != mm_sound_focus_unset_session_interrupt_callback())
			LOGW("failed to unset sessuib interrupt callback");
