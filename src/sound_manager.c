...
int sound_manager_set_session_interrupted_cb (sound_session_interrupted_cb callback, void *user_data)
{
	...
	if (g_session_interrupt_cb_table.is_registered == 0) {
		mm_sound_focus_set_session_interrupt_callback((mm_sound_focus_session_interrupt_cb)_focus_session_interrupt_cb);
		ret = mm_sound_add_device_connected_callback(SOUND_DEVICE_IO_DIRECTION_IN_MASK | SOUND_DEVICE_IO_DIRECTION_BOTH_MASK, (mm_sound_device_connected_cb)_earjack_unplugged_cb, NULL, &subs_id);
		if (ret)
			goto finish;
		g_session_interrupt_cb_table.is_registered = 1;
